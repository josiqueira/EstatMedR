...
plot(horas.estudo, nota.obtida,
     xlim=c(0,80),
     ylim=c(0,100),
     main=paste("Hipótese do estudante\n(dedicar mais não melhora a nota)",sep=""),
...
r <- which(df$horas>=42 & df$horas<=52)
...
for (i in 1:1000)
{
  ...
  plot(horas.estudo, nota.obtida,
       xlim=c(0,80),
       ylim=c(0,100),
       main=paste("Caso o estudante esteja certo\n",
                  "(dedicar mais não melhora a nota)\n",
                  "sem padrao ", sucesso,
                  " (",round((sucesso/(sucesso+falha))*100,2),"%)",
                  " x ",
                  falha," com padrao",sep=""),
       xlab="Horas de estudo na semana",
       ylab="Nota obtida no exame"
  )
