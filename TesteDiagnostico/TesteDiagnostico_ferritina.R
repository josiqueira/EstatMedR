...
if (exemplo == 'y')
{
  # exemplo da ferritina
  filename <- NA
  prevalencia <- 0.3137 #6875
  sensibilidade <- 0.9036 # 5847
  especificidade <- 0.8475 # 4576
  total <- 2579
  criterio <- 1e-4
  alfa <- 0.05
  exibetabela <- 0
...
if (exemplo=='n')
{
  ...
  if (total == 0)
  {
    ...
    for (total in 1:30000)
    {
      ...
      out <- epiR::epi.tests(tabela2x2, conf.level = 1-alfa)
      sumario <- summary(out)
  ...
  } else
  {
    ...
    if (exibetabela==1) {exibetabelatxt <- "sim"}
...
if (num_iteracoes > 0)
{
  ...
  out <- epiR::epi.tests(tabela2x2_propH1, conf.level = 1-alfa)
  ...
  out <- epiR::epi.tests(tabela2x2_propH0, conf.level = 1-alfa)
  ...
  for (i in 1:num_iteracoes)
  {
    ...
    out <- epiR::epi.tests(tabela2x2_tmp, conf.level = 1-alfa)
    sumario <- summary(out)
  ...
  maxx = max(d_aprev$x)	
  maxy <- max(d_aprev$y)	
  plot (d_aprev, main=paste("Distribuicao da Prevalencia Aparente",", n=",total,sep=""),	
  ...
  maxx = max(d_tprev$x)	
  maxy <- max(d_tprev$y)	
  plot (d_tprev, paste(main="Distribuicao de Prevalencia Observada",", n=",total,sep=""),	
  ...
  maxx = max(d_ppv$x)	
  maxy <- max(d_ppv$y)	
  plot (d_ppv, main=paste("Distribuicao de PPV",", n=",total,sep=""),	
  ...
  maxx = max(d_npv$x)	
  maxy <- max(d_npv$y)	
  plot (d_npv, main=paste("Distribuicao de NPV",", n=",total,sep=""),	
