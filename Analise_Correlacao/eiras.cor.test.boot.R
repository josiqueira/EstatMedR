                                 ...
                                 suppress.text=FALSE)
{
  ...
  df_boot <- df_boot[!is.na(df_boot$x),]
  df_boot <- df_boot[!is.na(df_boot$y),]
  n <- nrow(df_boot)
  x <- df_boot$x
  y <- df_boot$y
  if (sum(is.na(xlim))>0)
  ...
  )
  {
    ...
    if (B == 0)
    {
      ...
      r.txt <- "r"
      if (substr(method,1,1)=="s") {r.txt <- "s"}
      title <- paste(title, r.txt," = ",round(as.numeric(original.correlacao$estimate),3),
    ...
    if (B > 0)
    {
      ...
      for (b in 1:B)
      {
        ...
        if (add.regression==TRUE & substr(method,1,1)=="p")
        {
  ...
  )
  {
    ...
    if (B > 0)
    {
      # coleta variaveis de interesse
      c_b0 <- c()
      c_b1 <- c()
      c_R2 <- c()
      x_tp <- seq(min(x,na.rm=TRUE), max(x,na.rm=TRUE), length.out = 100)
      x_tp <- unique(x_tp)
      df_all_lines <- data.frame(matrix(ncol=length(x_tp),nrow=B))
      names(df_all_lines) <- x_tp
      for (b in 1:B)
      {
        df_sample <- dplyr::sample_n(df_boot, size=nrow(df_boot), replace=TRUE)
        names(df_sample) <- c("x","y")
        xb <- df_sample$x
        yb <- df_sample$y
        rls <- estimatr::lm_robust(yb ~xb)
        regressao <- summary(rls)
        c_b0 <- c(c_b0,regressao$coefficients[1,1])
        c_b1 <- c(c_b1,regressao$coefficients[2,1])
        c_R2 <- c(c_R2,regressao$r.squared)
        y_tp <- c_b0[b] + c_b1[b]*x_tp
        for (i in 1:length(y_tp))
        {
          df_all_lines[b,i] <- y_tp[i]
        }
      }
      # banda de IC95%
      x_ic <- x_tp
      y_ic_lower <- rep(NA,length(x_ic))
      y_ic_median <- rep(NA,length(x_ic))
      y_ic_upper <- rep(NA,length(x_ic))
      for (c in 1:ncol(df_all_lines))
      {
        q <- quantile(df_all_lines[,c], probs=c(0.025,0.5,0.975))
        y_ic_lower[c] <- q[1]
        y_ic_median[c] <- q[2]
        y_ic_upper[c] <- q[3]
      }
      ...
      title <- ""
      if (nchar(main)>0) {title <- paste(main,"\n",sep="")}
      q_b0 <- round(quantile(c_b0, probs=c(0.025,0.5,0.975)),3)
      q_b1 <- round(quantile(c_b1, probs=c(0.025,0.5,0.975)),3)
      q_R2 <- round(quantile(c_R2, probs=c(0.025,0.5,0.975)),3)
      title <- paste(title,
                     "y^ = ",
                     q_b0[2],"[",q_b0[1],",",q_b0[3],"] + ",
                     q_b1[2],"[",q_b1[1],",",q_b1[3],"]x",
                     "\nR^2 = ",
                     q_R2[2],"[",q_R2[1],",",q_R2[3],"]",
                     " (B = ",B,", n = ",n,")",
                     sep="")
    ...
    } else
    {
      ...
      if (method == "lm_robust")
      {
        # residuals (replacement for confidence band estimation)
        residuals <- c()
        for (i in 1:length(x))
        {
          ...
          {
            y.hat <- b0 + b1*x[i]
            residuals <- c(residuals,y[i]-y.hat)
          } else
          {
        ...
        names(residuals) <- seq(1:length(x))
        MSE = sum(residuals^2)/rls$df.residual
  ...
  {
    # grafico
    xg <- add.jitter(x,jitter) 
    xg[xg < min(x,na.rm = TRUE)] <- min(x,na.rm = TRUE)
    xg[xg > max(x,na.rm = TRUE)] <- max(x,na.rm = TRUE)
    yg <- add.jitter(y,jitter) 
    plot(xg, yg,
    ...
    {
      if (B>0) # bootstrapping
      {
        # all lines
        col_tp <- paste(col,"22",sep="")
        lwd <- 0.4
        for (b in 1:B)
        {
          lines(as.numeric(names(df_all_lines)), 
                df_all_lines[b,], 
                col=col_tp, lwd=lwd)
        }
        # banda de IC95%
      ...
      } else
      {
        ...
        lines(lowess(x, y), col=col, lty=2)
    ...
    if (add.regression == TRUE)
    {
      if (substr(method,1,1)=="p")
      {
        if (B > 0)
        {
          ...
          col_tp <- paste(friendlycolor(24),tp,sep="")
          lwd <- 0.4
          for (b in 1:B)
          {
    ...
    } else  
    {
      if (substr(method,1,1)=="p")
      {
        ellipseaxis(x,y,col=col,level=0.5)
      }
    }
  }
  ...
  if (suppress.text==FALSE)
  {
    ...
    )
    {
      ...
      if (B > 0)
      {
        # bootstrapping
        cat ("\n\t")
        if (substr(method,1,1)=="p") {cat ("Pearson"); r.txt <- "r"}
        if (substr(method,1,1)=="s") {cat ("Spearman"); r.txt <- "s"}
        cat ("'s ",r.txt," (bootstrapping with ",B," iterations):\n",sep="")
        cat ("\n",round((1-alpha)*100,0)," percent confidence interval:",sep="")
        cat ("\n\t",r[1],"\t",r[3], sep="")
        cat ("\ncor", sep="")
        cat ("\n\t",r[2], sep="")
        if (B == 0)
    ...
    )
    {
      if (B > 0)
      {
        cat("\nBootstrapping com ",B," reamostragens\n",sep="")
        cat("\t(mediana e IC95%)\n\n",sep="")
        cat("\tR^2 = coefic. de determinacao = ", q_R2[2]," [",q_R2[1],", ",q_R2[3],"]","\n", sep="")
        cat("\nReta de regressão mediana:\n",sep="")
        cat("\t",ylab, "^ = ", 
            q_b0[2]," [",q_b0[1],", ",q_b0[3],"] + ", 
            q_b1[2]," [",q_b1[1],", ",q_b1[3],"] . ",xlab,"\n",sep="")
      } else
      {
        print (regressao)
        cat("\n")
        cat("Desvio-padrao de ",ylab," = ",s,"\n",sep="")
        cat("Erro-padrao da estimativa de ",ylab," = ", SEE,"\n", sep="")
        cat("R multiplo = ",Rmult,"\n",sep="")
        cat("R^2 = coefic. de determinacao = ", R2,"\n", sep="")
