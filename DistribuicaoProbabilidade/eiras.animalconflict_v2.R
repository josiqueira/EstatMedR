...
fileconfig <- readline(prompt="Filename (xls or xlsx format): ")
...
txt <- as.character(df_config[which(
  df_config[,1]=="Max. Cycles"),1:3])
total.cycles <- as.integer(txt[2])
cat(" - ",txt[3],": ",total.cycles,"\n",sep="");
txt <- as.character(df_config[which(
  df_config[,1]=="Draw arrow each"),1:3])
draw.each <- as.integer(txt[2])
cat(" - ",txt[3],": ",draw.each,"\n",sep="");
    ...
    while(1)
    {
      ...
      if (cycle==1)
      {
        points(xy[1,1],xy[1,2],
               pch=21, cex=0.4, 
               col=eiras.friendlycolor(slow.to.fast[col]),
               bg=eiras.friendlycolor(slow.to.fast[col]))
      }
      lines(xy[,1],xy[,2], 
            col=eiras.friendlycolor(slow.to.fast[col]), lwd=1)
      # draw arrow  
...
posread <- as.numeric(gregexpr(pattern=".", fileread, fixed=TRUE))
if (posread > 0)
{
  fileread <- substr(fileread,start=1,stop=posread-1)
}
...
# openxlsx::write.xlsx(df_evolution, filewrite)

cat("- trajetorias guardadas em",filewrite,"\n")

filewrite <- eiras.ternary.replot(filewrite,png="y")
cat("replot em ",filewrite,"\n",sep="")
